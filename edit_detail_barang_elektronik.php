...
<html lang="en">
  ...
  <?php
    ...
    if(isset($_GET['Kode_peminjam'])){
        $detailelektronik = new LibraryDetailElektronik();
        $editDetailElektronik = $detailelektronik->editDetail($_GET['Kode_peminjam']);
        $edit = $editDetailElektronik->fetch(PDO::FETCH_OBJ);
    }
?>
    <!-- NAVBAR START -->
        ...
        <div class="card-body">
          ...
          <form action="" method="post">
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="kodeBarang" aria-describedby="emailHelp" name="Kode_peminjam" value="<?php echo $edit->Kode_peminjam; ?>">
            </div>
            <div class="mb-3">
                <label for="namaBarang" class="form-label">Peminjam</label>
                <input type="text" class="form-control" id="namaBarang" aria-describedby="emailHelp" name="peminjam" value="<?php echo $edit->peminjam; ?>">
            </div>
            <div class="mb-3">
                <label for="tanggal" class="form-label">Pemberi</label>
                <input type="text" class="form-control" id="tanggal" aria-describedby="emailHelp" name="pemberi" value="<?php echo $edit->pemberi; ?>">
            </div>
            <div class="mb-3">
                <label for="keterangan" class="form-label">Tanggal Kembali</label>
                <input type="date" class="form-control" id="keterangan" aria-describedby="emailHelp" name="tanggal_kembali" value="<?php echo $edit->tanggal_kembali; ?>">
            </div>
            <div class="mb-3">
                <label for="jumlah" class="form-label">Lokasi Di Pinjam</label>
                <input type="text" class="form-control" id="jumlah" aria-describedby="emailHelp" name="lokasi_dipinjam" value="<?php echo $edit->lokasi_dipinjam; ?>">
            </div>
...
if(isset($_POST['submit'])){
    $kodePeminjam = $_GET['Kode_peminjam'];
    ...
    $ubah = $detailelektronik->updateDetail($kodePeminjam,$kodeBarang,$peminjam,$pemberi,$tanggalKembali,$lokasiDipinjam);
    if($ubah = 'Edit Data Berhasil') {
        ...
        window.location.href='edit_detail_barang_elektronik.php'</script>";
    }
}
?>
